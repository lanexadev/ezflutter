  ...
  test(
    ...
    () async {
      final sandbox = await Directory.systemTemp.createTemp(
        'start_dart_create_integration_',
      );
      ...
      try {
        final exitCode =
            await StartDartCli(
              ...
              'generated_app',
              '--local-framework',
              '${repository.path}/packages/start_dart',
            ]);
...
Directory _findRepositoryRoot() {
  ...
  while (candidate.parent.path != candidate.path) {
    if (File(
      '${candidate.path}/packages/start_dart/pubspec.yaml',
    ).existsSync()) {
      return candidate;
    }
    candidate = candidate.parent;
  }
  throw StateError('Could not locate the START.DART workspace root.');
}
