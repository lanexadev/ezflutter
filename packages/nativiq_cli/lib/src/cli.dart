...
final class NativiqCli {
  ...
  Future<int> _create(List<String> args, bool jsonOutput) async {
    ...
    final staging = await parent.createTemp('.nativiq_${name}_');
    var promoted = false;
    try {
    ...
    for (final start in starts) {
      ...
      while (candidate.parent.path != candidate.path) {
        final workspacePackage = Directory(
          _join(candidate.path, 'packages/nativiq'),
        );
        if (File(_join(workspacePackage.path, 'pubspec.yaml')).existsSync()) {
          return workspacePackage.path;
        }
        final siblingPackage = Directory(_join(candidate.path, '../nativiq'));
...
const _usage = '''Nativiq — AI-native Flutter application toolkit

Usage:
  nativiq create <name> [--output <path>] [--org <domain>] [--description <text>] [--local-framework <path>] [--json]
  nativiq doctor [--json]
  nativiq add feature <name> [--project <path>] [--json]
  nativiq add model <feature> <name> [--project <path>] [--json]''';
